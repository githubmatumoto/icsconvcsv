...
## 2026-1-29: Version:3.1.1
(内部メモ:subversion revision 2204, フォルダv3.1)

- ライブラリのバージョン確認を行う変数VERSIONの更新忘れ。3.0から3.1に
  更新。このため、3.1は公開停止しています。

- UTF-8の読み替え表更新。サンプルのファイル名などが間違えてたので、修
  正。

- 多くのLinux環境ではライブラリtzdataを導入しなくてもTimeZoneを指定す
  る-Tオプションが動作するため、INSTALL.mdにtzdataの導入について記載し
