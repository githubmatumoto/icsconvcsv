...
**テキストエディタで閲覧される方へ**:本ファイルはMARKDOWN言語で記述さ
...
Outlook(Web版)ではスケジュール表をエクスポートする場合はICS形式しか選
...
また、Outlook(classic)ではCSVの出力は可能なため、将来的にOutlook(Web)
でも対応してくれないかなと思ってるのですが、、もしOutlook(Web)でCSVの
出力が可能になりましたら、本ツールの保守は終了します。

(職場向け追記)本ツールは職場での本採用は見送られましたので、いつまで保
守するかわかりませんが、本採用のツールは繰り返しスケジュール
(RRULE+RECURRENCE-ID)の処理が甘いため、変換に失敗した場合は、本ツール
をお使いください。本採用のツールで正しく処理ができるようになった場合は、
本ツールの保守は終了します。

## 1.2: 依存ライブラリ
ライブラリ vobjectを使わせていただいてます。バージョンは0.9.9で動作確認
しています。
...
※前述の「2: 動作環境およびインストール方法」に述べた方法で初期設定を
行ってない場合は、初期設定を行ってください。

本プログラムを利用する前に、``毎回''必要な設定や確認事項があります。
...
※作者の職場の皆様は「3.3.2: 実行例2: kiroku.py」をお使いください。

### 3.3.1: 実行例1: icsconvcsv.py
...
します。(ICSファイルに2025年9月分のスケジュールが記載されてない場合は
空のファイルが生成されます。)
...
### 3.3.3: 旧版の互換コマンド: ics2gacsv.py

過去に公開してた「ics2gacsv.py」の互換コマンドを同梱しています。
...
でいろいろバグが出ました。一応既知の例はデバッグしましたが、未知のバグ
があるかもしれません。
